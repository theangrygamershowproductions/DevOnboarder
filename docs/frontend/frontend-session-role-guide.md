...
## 🛠 Admin Management UI
Admin-only React components live under `src/components/admin/`.
They interact with `/api/admin/users` and `/api/admin/verification` and
require the `X-Is-Admin: true` header when testing locally.
