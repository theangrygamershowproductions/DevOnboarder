...
for file in "$WORKFLOW_DIR"/*.yml "$WORKFLOW_DIR"/*.yaml; do
    ...
    if grep -P '[\x{1F600}-\x{1F64F}]|[\x{1F300}-\x{1F5FF}]|[\x{1F680}-\x{1F6FF}]|[\x{2600}-\x{26FF}]|[\x{2700}-\x{27BF}]|||||GROW:|📥|LINK:|🐛|||TARGET:||||' "$file" 2>/dev/null; then
    ...
    if grep -B3 -A3 'EOF' "$file" | grep -E 'echo|comment.*body' 2>/dev/null; then
        ...
        echo " Here-doc near echo/comment context in $file"
...
if [ "$total_violations" -gt 0 ]; then
    ...
    echo "ENFORCEMENT  No terminal output policy violations found"
