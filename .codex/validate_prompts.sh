...
if [ ! -d ".codex" ]; then
    echo "  .codex directory not found - this is optional for now"
fi
...
validate_yaml_frontmatter() {
    ...
    if ! echo "$frontmatter" | grep -q "^author: \"TAGS Engineering\""; then
        echo " Incorrect author field - should be 'TAGS Engineering'"
        printf "File: %s\n" "$file"
        return 1
    fi

    if ! echo "$frontmatter" | grep -q "^project: \"core-instructions\""; then
        echo " Incorrect project field - should be 'core-instructions'"
        printf "File: %s\n" "$file"
        return 1
    fi

    if ! echo "$frontmatter" | grep -q "^codex_runtime: false"; then
        echo " Incorrect codex_runtime - should be 'false' for draft mode"
        printf "File: %s\n" "$file"
        return 1
    fi
    ...
    if echo "$frontmatter" | grep -q "^codex_type: \"AGENT\""; then
        if ! echo "$frontmatter" | grep -q "^discord_role_required:"; then
            echo " Missing discord_role_required field for AGENT type"
            printf "File: %s\n" "$file"
            return 1
        fi
        if ! echo "$frontmatter" | grep -q "^authentication_required: true"; then
            echo " Missing or incorrect authentication_required field for AGENT type"
            printf "File: %s\n" "$file"
            return 1
        fi
    fi
...
validate_integration_status() {
    ...
    if [[ "$file" == *"agent_"* ]]; then
        if ! grep -q "DRAFT.*Not Live" "$file"; then
            echo " Agent file missing draft mode warning"
            printf "File: %s\n" "$file"
        fi

        if ! grep -q "Related Integration Docs" "$file"; then
            echo " Missing integration documentation links"
            printf "File: %s\n" "$file"
        fi
    fi
    ...
    if grep -q "codex_runtime.*true" "$file"; then
        echo " Live runtime detected - should be false in draft mode"
        printf "File: %s\n" "$file"
        return 1
    fi

    echo " Integration status valid"
    printf "File: %s\n" "$file"
}
...
validate_naming_convention() {
    ...
    # Check checklist files
        ...
        echo "  Non-standard filename (may be intentional): $file"
    fi
}
...
validate_title_format() {
    ...
    if [[ "$title" =~ ^[A-Z]:[A-Z]:[A-Z]$ ]]; then
        ...
        echo "  Title format could be enhanced for Codex: $title"
