...
# Calculate project root from framework script location
...
HEALTH_RESULT=$(bash "$PROJECT_ROOT/scripts/assess_pr_health.sh" "$PR_NUMBER" 2>&1 | tee -a "$LOG_FILE")
...
PATTERN_RESULT=$(bash "$PROJECT_ROOT/frameworks/ci-cd-automation/analysis/analyze_ci_patterns.sh" "$PR_NUMBER" 2>&1 | tee -a "$LOG_FILE")
PATTERN_EXIT_CODE=$?
...
if DECISION_RESULT=$(bash "$PROJECT_ROOT/frameworks/ci-cd-automation/automation/pr_decision_engine.sh" "$PR_NUMBER" 2>&1 | tee -a "$LOG_FILE"); then
...
if [ "$ACTION_MODE" = "full-auto" ]; then
    ...
    # Re-evaluate health
    UPDATED_HEALTH=$(bash "$PROJECT_ROOT/scripts/assess_pr_health.sh" "$PR_NUMBER" 2>/dev/null | grep "PR Health Score:" | sed 's/.*: \([0-9]*\)%.*/\1/' || echo "0")
...
if [ "$ACTION_MODE" = "analyze" ]; then
    ...
    echo "bash frameworks/ci-cd-automation/automation/automate_pr_process.sh $PR_NUMBER execute"
    echo ""
    echo -e "${BLUE}🤖 For full automation:${NC}"
    echo "bash frameworks/ci-cd-automation/automation/automate_pr_process.sh $PR_NUMBER full-auto"
fi
