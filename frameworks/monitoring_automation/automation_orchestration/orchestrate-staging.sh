...
if command -v require_tokens >/dev/null 2>&1; then
    if ! require_tokens "STAGING_ORCHESTRATION_BOT_KEY"; then
        echo "ERROR: Cannot proceed without required tokens for staging orchestration"
        echo "TIP: Staging orchestration requires API access to orchestration service"
        exit 1
    fi
fi
...
if [ -z "${ORCHESTRATION_KEY}" ]; then
    ...
    echo "TIP: Please ensure STAGING_ORCHESTRATION_BOT_KEY is set in your .tokens file"
    exit 1
fi
