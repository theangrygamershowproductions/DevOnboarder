...
echo -e "${YELLOW}REPORT: STEP 1: Health Assessment"
...
echo -e "${YELLOW}CHECK: STEP 2: CI Pattern Analysis"
...
if [ "$ACTION_MODE" = "full-auto" ]; then
    echo -e "${YELLOW}DEPLOY: STEP 5: Auto-Merge Evaluation"
    ...
    if [ "$AUTO_MERGE" = true ]; then
        echo -e "${GREEN}🎉 AUTO-MERGE CONDITIONS MET"
...
echo "==============================="
echo -e "report "Health Score: ${HEALTH_SCORE}%"
echo -e "target "Recommendation: $RECOMMENDATION"
echo -e "tool "Fixes Applied: $FIXES_APPLIED"
echo -e "check "Full Report: reports/pr_${PR_NUMBER}_automation_report.md"
echo -e "note "Detailed Log: $LOG_FILE"

if [ "$ACTION_MODE" = "analyze" ]; then
    echo ""
    echo -e "${BLUE}DEPLOY: To execute automated fixes:"
