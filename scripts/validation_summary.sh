...
if command -v npx >/dev/null 2>&1; then
    ...
    if [ -n "$MARKDOWN_OUTPUT" ]; then
        ...
        if [ "$MARKDOWN_ERRORS" -gt 0 ]; then
            echo "FAILED Found $MARKDOWN_ERRORS markdown violations:"
            ...
            echo "SUCCESS No markdown violations found"
        fi
    else
        echo "SUCCESS No markdown violations found"
    fi
else
    echo "WARNING  markdownlint not available - skipped"
fi
echo

# 2. Terminal Output Policy Summary
echo "SYMBOL TERMINAL OUTPUT POLICY"
echo "-------------------------"
if [ -f "$REPO_ROOT/scripts/validate_terminal_output_simple.sh" ]; then
    ...
    if [ "$TERMINAL_ERRORS" -gt 0 ]; then
        echo "FAILED Found $TERMINAL_ERRORS terminal policy violations:"
        ...
        echo "SUCCESS No terminal policy violations found"
    fi
else
    echo "WARNING  Terminal validation script not found - skipped"
fi
echo

# 3. Agent Certification Summary
echo "Bot AGENT CERTIFICATION"
echo "----------------------"
if [ -f "$REPO_ROOT/scripts/validate_agent_certification.sh" ]; then
    ...
    if echo "$AGENT_OUTPUT" | grep -q "CERTIFICATION INCOMPLETE"; then
        ...
        echo "FAILED Found $AGENT_ERRORS agent certification issues:"
        echo "$AGENT_OUTPUT" | grep "ERROR:" | head -3
        TOTAL_ERRORS=$((TOTAL_ERRORS + AGENT_ERRORS))
    else
        echo "SUCCESS All agents certified"
    fi
else
    echo "WARNING  Agent validation script not found - skipped"
fi
echo
...
if [ "$TOTAL_ERRORS" -eq 0 ]; then
    ...
    echo
    echo "SYMBOL Quick Fix Commands:"
