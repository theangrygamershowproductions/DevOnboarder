...
echo -e "${GREEN}SYMBOL DevOnboarder Token Policy Enforcement & Cleanup Validation${NC}"
echo "==============================================================="
...
log_violation() {
    ...
    case "$severity" in
        "CRITICAL")
            echo -e "${RED}FAILED CRITICAL: $message${NC}"
            echo "   File: $file"
            [ -n "$line_number" ] && echo "   Line: $line_number"
            ((VIOLATIONS_FOUND++))
            ;;
        "WARNING")
            echo -e "${YELLOW}WARNING  WARNING: $message${NC}"
            echo "   File: $file"
            [ -n "$line_number" ] && echo "   Line: $line_number"
            ((WARNINGS_FOUND++))
            ;;
        "INFO")
            echo -e "${BLUE}ℹSYMBOL  INFO: $message${NC}"
            echo "   File: $file"
            ;;
    esac
}
...
activate_virtual_env() {
    ...
    echo -e "${BLUE}SYMBOL Virtual environment activated for Python tools${NC}"
}

# Function to check registry availability and structure
check_token_registry() {
    echo -e "${BLUE}SYMBOL Checking token scope registry...${NC}"
    ...
    echo "SUCCESS Registry file exists: $REGISTRY_FILE"
...
try:
    ...
    exit(1)
"; then
            echo "SUCCESS Registry has valid YAML structure"
        else
...
scan_workflows_for_violations() {
    echo -e "${BLUE}SEARCH Scanning GitHub workflows for token policy violations...${NC}"
...
scan_agent_docs_for_token_usage() {
    echo -e "${BLUE}SYMBOL Scanning agent documentation for token usage patterns...${NC}"
    ...
    while IFS= read -r agent_file; do
        ...
        if grep -n -i "token" "$agent_file" >/dev/null 2>&1; then
            ...
            echo "  EDIT Found token references in documentation"
        fi
...
run_comprehensive_token_audit() {
    echo -e "${BLUE}SYMBOL Running comprehensive token audit...${NC}"
    ...
    if python "$AUDIT_SCRIPT" \
        ...
        echo "SUCCESS Token audit completed successfully"
...
check_token_cleanup() {
    echo -e "${BLUE}EMOJI Checking for token-related cleanup requirements...${NC}"
    ...
    if [ ${#temp_token_files[@]} -gt 0 ]; then
        ...
        echo "SUCCESS No unexpected token-related files found"
    fi
    ...
    if [ ${#env_files[@]} -gt 0 ]; then
        ...
        for env_file in "${env_files[@]}"; do
            if grep -q "TOKEN" "$env_file" 2>/dev/null; then
                echo "  EDIT $env_file contains token references"
...
check_potato_policy_integration() {
    echo -e "${BLUE}POTATO Checking Enhanced Potato Policy integration with token governance...${NC}"

    # Check for Potato Policy enforcement script
    if [ -f "scripts/check_potato_ignore.sh" ]; then
        echo "SUCCESS Potato Policy enforcement script found"

        # Check if it's being run (should be in workflows)
        if grep -r "check_potato_ignore" .github/workflows/ >/dev/null 2>&1; then
            echo "SUCCESS Potato Policy integrated into CI workflows"
        else
    ...
    for ignore_file in "${ignore_files[@]}"; do
        if [ -f "$ignore_file" ]; then
            if grep -q "Potato" "$ignore_file"; then
                echo "SUCCESS Potato entries found in $ignore_file"
            else
...
generate_enforcement_report() {
    echo -e "${BLUE}STATS Generating comprehensive token policy enforcement report...${NC}"
...
$(if [ "$VIOLATIONS_FOUND" -eq 0 ]; then
    echo "SUCCESS **COMPLIANT** - No critical violations detected"
else
    echo "FAILED **NON-COMPLIANT** - $VIOLATIONS_FOUND critical violations detected"
fi)

## Key Findings

### Token Registry Status
- Registry File: $([ -f "$REGISTRY_FILE" ] && echo "SUCCESS Present" || echo "FAILED Missing")
- YAML Structure: $(python -c "import yaml; yaml.safe_load(open('$REGISTRY_FILE'))" 2>/dev/null && echo "SUCCESS Valid" || echo "FAILED Invalid")
...
- Enforcement Script: $([ -f "scripts/check_potato_ignore.sh" ] && echo "SUCCESS Present" || echo "FAILED Missing")
- CI Integration: $(grep -r "check_potato_ignore" .github/workflows/ >/dev/null 2>&1 && echo "SUCCESS Active" || echo "FAILED Missing")

### Virtual Environment Compliance
- Virtual Environment: $([ -d ".venv" ] && echo "SUCCESS Available" || echo "FAILED Missing")
- Audit Script: $([ -f "$AUDIT_SCRIPT" ] && echo "SUCCESS Available" || echo "FAILED Missing")
...
EOF
    ...
    echo "SUCCESS Enforcement report generated: $report_file"
}
...
main() {
    ...
    # Final summary
    echo -e "${BLUE}SYMBOL Validation Summary${NC}"
    echo "=================="
    ...
    if [ "$VIOLATIONS_FOUND" -eq 0 ]; then
        echo -e "${GREEN}SUCCESS SUCCESS: No critical policy violations detected${NC}"
        echo -e "Token governance appears to be properly implemented."
        echo ""
        echo "STATS Full report: $TOKEN_AUDIT_DIR/enforcement-validation-report.md"
        echo "EDIT Detailed log: $LOG_FILE"
        return 0
    else
        echo -e "${RED}FAILED FAILURE: $VIOLATIONS_FOUND critical policy violations detected${NC}"
