#!/bin/bash
# Terminal Output Policy Enforcement Script
...
echo "Starting Terminal Output Policy Validation"
...
find "$WORKFLOW_DIR" -name "*.yml" -o -name "*.yaml" | while IFS= read -r file; do
    ...
    # Check for command substitution in echo statements
    if grep -E 'echo.*\$\(' "$file" 2>/dev/null; then
