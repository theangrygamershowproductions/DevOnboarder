...

{
    ...
    if [ ! -f .git/hooks/pre-commit ]; then
        ...
        if [ "$CI" = "true" ] || [ "$GITHUB_ACTIONS" = "true" ]; then
            echo " Pre-commit hooks not installed in CI environment"
    ...
    if ! command -v pre-commit >/dev/null 2>&1; then
        if [ "$CI" = "true" ] || [ "$GITHUB_ACTIONS" = "true" ]; then
            echo " pre-commit command not found in CI (expected)"
        ...
        echo " pre-commit executable found at: $(which pre-commit)"
    ...
    git add "$TEST_FILE" 2>/dev/null || {
        echo " Could not stage test file for validation"
        rm -f "$TEST_FILE"
    }
    ...
    if git ls-files --cached | grep -q "$TEST_FILE"; then
        ...
        echo " Skipping functional test (could not stage test file)"
    fi
    ...
    for file in "${REQUIRED_FILES[@]}"; do
        if [[ -f "$file" ]]; then
            ...
            echo " $file missing (quality configuration may be incomplete)"
        fi
    done
    ...
    if [[ -f "scripts/safe_commit.sh" && -x "scripts/safe_commit.sh" ]]; then
        echo " safe_commit.sh exists and is executable"
    else
        echo " scripts/safe_commit.sh missing or not executable"
