...
detect_context() {
    ...
    log_message "$BLUE" "SEARCH $SCRIPT_NAME: Running in $CONTEXT_MODE context"
}
    ...
    for pattern in $patterns; do
        ...
        while IFS= read -r -d '' file; do
            if [[ -e "$file" ]]; then
                ...
                log_message "$RED" "FAILED VIOLATION [$pattern_name]: $file"
...
check_venv_compliance() {
    log_message "$BLUE" "SYMBOL Checking virtual environment compliance..."

    # Check if virtual environment exists
    if [[ ! -d ".venv" ]]; then
        if [[ "$CONTEXT_MODE" == "LOCAL" ]]; then
            log_message "$YELLOW" "WARNING  No .venv directory found"
    ...
    if [[ -d "__pycache__" ]] && [[ "$CONTEXT_MODE" == "LOCAL" ]]; then
        log_message "$YELLOW" "WARNING  Root __pycache__ detected - ensure using virtual environment"
        return 1
    fi

    log_message "$GREEN" "SUCCESS Virtual environment compliance OK"
    return 0
}
...
check_root_pollution() {
    ...
    for pattern_name in "${!ARTIFACT_PATTERNS[@]}"; do
        log_message "$BLUE" "SYMBOL Checking $pattern_name artifacts..."
...
check_ci_specific_artifacts() {
    log_message "$BLUE" "CONFIG Checking CI-specific artifacts..."
    ...
    if [[ -d ".github/workflows" ]]; then
        ...
        for artifact in "test-results" "coverage-reports" "build-logs"; do
            if [[ -e "$artifact" ]]; then
                log_message "$RED" "FAILED CI VIOLATION: $artifact should be in logs/ or removed"
...
suggest_enhanced_cleanup() {
    log_message "$YELLOW" "CONFIG $SCRIPT_NAME: Enhanced cleanup suggestions:"
    echo ""

    if [[ ${#CLEANUP_SUGGESTIONS[@]} -gt 0 ]]; then
        log_message "$BLUE" "SYMBOL Specific cleanup commands:"
        printf '%s\n' "${CLEANUP_SUGGESTIONS[@]}"
        echo ""
    fi

    log_message "$BLUE" "DEPLOY Automated cleanup options:"
    ...
    if [[ "$CONTEXT_MODE" == "LOCAL" ]]; then
        log_message "$PURPLE" "IDEA DevOnboarder best practices:"
...
auto_cleanup() {
    ...
    log_message "$BLUE" "SYMBOL Creating backup in $backup_dir"
    ...
    for file in "${VIOLATION_FILES[@]}"; do
        if [[ -e "$file" ]]; then
            if [[ -f "$file" ]] && [[ $(stat -f%z "$file" 2>/dev/null || stat -c%s "$file" 2>/dev/null || echo 0) -gt 1048576 ]]; then
                ...
                log_message "$BLUE" "SYMBOL Backed up large file: $file"
            fi

            rm -rf "$file"
            log_message "$GREEN" "SYMBOL  Removed: $file"
        fi
    done

    log_message "$GREEN" "SUCCESS Automated cleanup complete"
}

# Interactive cleanup wizard (Phase 3.4 preview)
cleanup_wizard() {
    log_message "$PURPLE" "WIZARD Interactive Cleanup Wizard"
    echo ""

    if [[ ${#VIOLATION_FILES[@]} -eq 0 ]]; then
        log_message "$GREEN" "SUCCESS No artifacts found to clean!"
        return 0
    fi
    ...
    for i in "${!VIOLATION_FILES[@]}"; do
        ...
        case "$response" in
            [Yy]*)
                rm -rf "$file"
                log_message "$GREEN" "SUCCESS Removed: $file"
                ;;
            [Qq]*)
    ...
    log_message "$GREEN" "SYMBOL Cleanup wizard complete!"
}
...
main() {
    ...
    if [[ $violations_found -eq 0 ]]; then
        log_message "$GREEN" "SUCCESS $SCRIPT_NAME: Repository root is clean!"
        ...
        log_message "$YELLOW" "SYMBOL Root pollution violates DevOnboarder Enhanced Potato Policy:"
        ...
        suggest_enhanced_cleanup
        echo ""
        log_message "$RED" "SYMBOL Fix root pollution before proceeding"
        exit 1
    fi
}
...
case "${1:-check}" in
    ...
    --auto-clean|auto-clean)
        ...
        if [[ $violations_found -gt 0 ]]; then
            ...
            log_message "$GREEN" "SUCCESS No cleanup needed"
        fi
        ;;
    --wizard|wizard)
        check_root_pollution
        ...
        if [[ $violations_found -gt 0 ]]; then
            ...
            log_message "$GREEN" "SUCCESS Repository is already clean"
        fi
        ;;
    --patterns|patterns)
        log_message "$BLUE" "SYMBOL Enhanced artifact detection patterns:"
        for pattern_name in "${!ARTIFACT_PATTERNS[@]}"; do
