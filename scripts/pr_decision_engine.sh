...
if HEALTH_OUTPUT=$(bash scripts/assess_pr_health.sh "$PR_NUMBER" 2>/dev/null); then
    ...
    echo "  Health assessment failed, using estimated score"
...
echo "  Attempting to fetch PR data from GitHub..."
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
    if PR_DATA=$(gh pr view "$PR_NUMBER" --json commits,changedFiles,additions,title,body 2>/dev/null); then
        ...
        echo "  GitHub CLI available but failed to fetch PR data"
        COMMIT_COUNT=0
        CHANGED_FILES=0
        ADDITIONS=0
    ...
    echo "  GitHub CLI not available or not authenticated, using defaults"
    COMMIT_COUNT=2
    CHANGED_FILES=5
    ADDITIONS=100
    PR_DATA='{"title":"Potato Policy Enforcement","body":"Focused PR for Potato.md ignore policy implementation"}'
...
case "$RECOMMENDATION" in
    ...
    "MINOR FIXES THEN MERGE")
        ...
        echo "  FAST: Quick fixes will meet our quality standards"
        echo "  GROW: ROI is positive for completing to 95%"
        ;;
    "EVALUATE CORE MISSION COMPLETION")
        ...
        echo "     ${HEALTH_SCORE}% is significantly below our 95% standard"
        ;;
...
case "$RECOMMENDATION" in
    ...
    "EVALUATE CORE MISSION COMPLETION")
        ...
        echo "  3. 🚨  ${HEALTH_SCORE}% << 95% standard - exceptional justification needed"
...
echo
echo "  QUALITY REMINDER: Our standard is 95% health score"
