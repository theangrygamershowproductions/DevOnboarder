...
echo "SYMBOL DevOnboarder Discord Environment Setup"
echo "========================================="
...
echo "CONFIG Environment: $DEPLOY_ENV"
echo ""
...
setup_bot_config() {
    ...
    if [[ ! -d "$BOT_DIR" ]]; then
        echo "FAILED Bot directory not found: $BOT_DIR"
        exit 1
    fi
    ...
    if [[ "$DEPLOY_ENV" == "dev" ]]; then
        ...
        echo "LINK Development Discord Server: TAGS: DevOnboarder ($DISCORD_DEV_GUILD_ID)"
    elif [[ "$DEPLOY_ENV" == "prod" ]]; then
        create_env_file "prod" "$DISCORD_PROD_GUILD_ID" "$DISCORD_PROD_WEBHOOK"
        echo "LINK Production Discord Server: TAGS: C2C ($DISCORD_PROD_GUILD_ID)"
    else
        echo "WARNING  Unknown environment: $DEPLOY_ENV"
    ...
    if [[ "$DEPLOY_ENV" == "dev" || "$DEPLOY_ENV" == "prod" ]]; then
        echo "LINK Creating main .env file for $DEPLOY_ENV environment..."
        cp "${BOT_DIR}/.env.${DEPLOY_ENV}" "$main_env_file"
        echo "SUCCESS Main .env file updated for $DEPLOY_ENV environment"
    fi
}

# Function to validate configuration
validate_config() {
    echo "SEARCH Validating Discord configuration..."

    local env_file="${BOT_DIR}/.env"
    if [[ ! -f "$env_file" ]]; then
        echo "FAILED Main .env file not found: $env_file"
        return 1
    fi
    ...
    if [[ ${#missing_vars[@]} -gt 0 ]]; then
        echo "FAILED Missing required variables: ${missing_vars[*]}"
        return 1
    fi
    ...
    echo ""
    echo "SYMBOL Current Configuration:"
...
setup_role_mapping() {
    echo "SYMBOL Setting up Discord role mapping..."
...
EOF
    ...
    echo "SUCCESS Role mapping configuration created: $roles_config"
}

# Function to create deployment scripts
create_deployment_scripts() {
    echo "DEPLOY Creating deployment scripts..."
...
if [[ ! -d "node_modules" ]]; then
    echo "SYMBOL Installing bot dependencies..."
    npm install
fi

# Build if needed
if [[ ! -d "dist" ]] || [[ "src/" -nt "dist/" ]]; then
    echo "SYMBOL Building bot..."
    npm run build
fi
...
if [[ ! -f ".env.prod" ]]; then
    echo "FAILED Production environment file not found: .env.prod"
...
echo "DEPLOY Starting bot in production mode..."
npm start
EOF
    ...
    echo "SUCCESS Deployment scripts created:"
...
test_discord_connection() {
    ...
    if [[ "${LIVE_TRIGGERS_ENABLED:-false}" == "true" ]]; then
        ...
        if [[ -n "$webhook_url" ]] && [[ "$webhook_url" != "changeme" ]]; then
            echo "LINK Testing webhook connectivity..."
            if curl -s -o /dev/null -w "%{http_code}" "$webhook_url" | grep -q "2[0-9][0-9]"; then
                echo "SUCCESS Webhook connectivity test passed"
            else
                echo "WARNING  Webhook connectivity test failed (may be expected in dry-run mode)"
            fi
        fi
    else
        echo "EMOJI Skipping webhook test (dry-run mode active)"
    fi
}
...
display_summary() {
    ...
    echo "======================================"
    echo ""
    echo "SYMBOL Configuration Summary:"
    ...
    echo "   - Bot Connection: ⏸SYMBOL Awaiting token configuration"
}

# Main execution
main() {
    echo "SYMBOL Starting Discord environment setup for: $DEPLOY_ENV"
    echo ""
    ...
    echo ""
    echo "SUCCESS Discord environment setup completed successfully!"
}
