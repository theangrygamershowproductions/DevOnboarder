...
ssh-keygen -y -f "$SSH_KEY_PATH" > /dev/null 2>&1 && echo " VALID" || echo " INVALID"
...
ssh-keygen -y -f test_key_1 > /dev/null 2>&1 && echo " Method 1: VALID" || echo " Method 1: INVALID"

echo ""
echo "Testing Method 2: printf with %s"
printf '%s' "$SSH_KEY_CONTENT" > test_key_2
chmod 600 test_key_2
ssh-keygen -y -f test_key_2 > /dev/null 2>&1 && echo " Method 2: VALID" || echo " Method 2: INVALID"

echo ""
echo "Testing Method 3: printf with %s and newline"
printf '%s\n' "$SSH_KEY_CONTENT" > test_key_3
chmod 600 test_key_3
ssh-keygen -y -f test_key_3 > /dev/null 2>&1 && echo " Method 3: VALID" || echo " Method 3: INVALID"
...
ssh-keygen -y -f test_key_4 > /dev/null 2>&1 && echo " Method 4: VALID" || echo " Method 4: INVALID"
...
ssh-keygen -y -f test_key_5 > /dev/null 2>&1 && echo " Method 5: VALID" || echo " Method 5: INVALID"
