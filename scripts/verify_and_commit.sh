...
main() {
    log_and_display "CONFIG DevOnboarder Pre-commit Verification and Commit Script"
    ...
    if [ ! -d ".venv" ]; then
        log_and_display "FAILED ERROR: Virtual environment .venv not found"
        log_and_display "Run: python -m venv .venv && source .venv/bin/activate && pip install -e .[test]"
        exit 1
    fi

    log_and_display "SYMBOL Activating virtual environment..."
    # shellcheck source=/dev/null
    source .venv/bin/activate

    # Verify Python and pip are from virtual environment
    log_and_display "SYMBOL Python executable: $(which python)"
    log_and_display "SYMBOL Pip executable: $(which pip)"

    # Ensure logs directory exists
    mkdir -p logs

    # Validate YAML configuration
    log_and_display "EDIT Validating pre-commit YAML configuration..."
    if python -c "import yaml; yaml.safe_load(open('.pre-commit-config.yaml'))" 2>&1 | tee -a "$LOG_FILE"; then
        log_and_display "SUCCESS YAML configuration is valid"
    else
        log_and_display "FAILED YAML configuration has syntax errors"
        exit 1
    fi

    # Check git status before proceeding
    log_and_display "STATS Current git status:"
    git status --short | tee -a "$LOG_FILE"

    # Clean all test artifacts before validation for clean state
    log_and_display "EMOJI Cleaning all test artifacts before validation..."
    ...
    if [ -f "scripts/clean_pytest_artifacts.sh" ]; then
        ...
        log_and_display "   WARNING  Pytest cleanup script not found"
    fi
    log_and_display ""

    # Test pre-commit hooks
    log_and_display "EMOJI Testing pre-commit hooks with comprehensive logging..."
    ...
    if pre-commit run --all-files 2>&1 | tee "$PRECOMMIT_LOG"; then
        ...
        log_and_display "FAILED Pre-commit hooks failed. Check $PRECOMMIT_LOG for details"
    ...
    # Stage all changes
    log_and_display "SYMBOL Staging all changes..."
    git add .

    # Show what will be committed
    log_and_display "SYMBOL Files to be committed:"
    git diff --cached --name-only | tee -a "$LOG_FILE"

    # Generate commit message if not provided
    if [ $# -eq 0 ]; then
        log_and_display "SYMBOL No commit message provided. Using default message..."
...
- Ensure all pre-commit hooks output logs to logs/ directory for troubleshooting"
    ...
    # Attempt commit
    log_and_display "SYMBOL Committing changes with message:"
    echo "$COMMIT_MSG" | tee -a "$LOG_FILE"
    log_and_display ""

    if git commit -m "$COMMIT_MSG" 2>&1 | tee -a "$LOG_FILE"; then
        log_and_display "SYMBOL Commit successful!"

        # Show latest commit
        log_and_display "EDIT Latest commit:"
        git log --oneline -1 | tee -a "$LOG_FILE"

        # Show log summary
        log_and_display "STATS Generated log files:"
        find logs/ -type f -printf "%T@ %p\n" 2>/dev/null | sort -n | tail -10 | cut -d' ' -f2- | while read -r file; do ls -la "$file"; done | tee -a "$LOG_FILE"

        log_and_display ""
        log_and_display "SUCCESS All checks passed! Ready for push."
        log_and_display "SYMBOL Next steps:"
        ...
        log_and_display "FAILED Commit failed. Check logs for details"
        exit 1
    fi
