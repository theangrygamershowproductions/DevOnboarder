...
echo "CONFIG DevOnboarder CI Failure Fix"
echo "============================="

# 1. Fix Environment Variables
echo "EDIT Fixing environment variables..."

# Ensure .env.dev has all required values from .env.example
if [ ! -f .env.dev ]; then
    echo "FAILED .env.dev not found. Creating from .env.example..."
    cp .env.example .env.dev
fi

# Run the secret generation script
echo "SYMBOL Generating missing secrets..."
bash scripts/generate-secrets.sh

# 2. Install missing dependencies that often cause CI failures
echo "SYMBOL Installing missing Python tools..."
# shellcheck source=/dev/null
source .venv/bin/activate

# Install tools commonly missing in CI
pip install pip-audit black openapi-spec-validator mypy ruff 2>/dev/null || {
    echo "WARNING  Some tools failed to install - this is often expected in offline mode"
}

# 3. Setup Node.js dependencies
echo "SYMBOL Installing Node.js dependencies..."
if [ -d "bot" ] && [ -f "bot/package.json" ]; then
    npm ci --prefix bot --silent || {
        echo "WARNING  Bot npm install failed - may be offline"
    }
fi

if [ -d "frontend" ] && [ -f "frontend/package.json" ]; then
    npm ci --prefix frontend --silent || {
        echo "WARNING  Frontend npm install failed - may be offline"
    }
fi

# 4. Fix common Docker issues
echo "SYMBOL Checking Docker setup..."
if command -v docker >/dev/null 2>&1; then
    if docker compose -f docker-compose.ci.yaml config >/dev/null 2>&1; then
        echo "SUCCESS Docker compose configuration valid"
    else
        echo "FAILED Docker compose configuration has issues"
    fi
else
    echo "WARNING  Docker not available - some CI steps will be skipped"
fi

# 5. Run diagnostics in different modes
echo "SEARCH Running diagnostics..."

# Test without services (like early CI stages)
export TAGS_MODE=false
echo "Testing with TAGS_MODE=false (no services expected):"
python -m diagnostics 2>&1 | head -n 10 || echo "WARNING  Some diagnostics failed (expected without running services)"

# 6. Check for common file issues
echo "FOLDER Checking file permissions and existence..."
...
echo "EMOJI Testing core functionality..."

# Run a minimal test suite
pytest tests/test_smoke.py -v --tb=short 2>/dev/null || echo "WARNING  Smoke tests failed - may need services running"
