...
echo "$(blue "🔍 DevOnboarder Automerge Health Check")"
echo "========================================"
...
if PROTECTION_RULES=$(gh api "repos/$REPO_FULL/branches/main/protection" 2>/dev/null); then
    ...
    if [ "$REQUIRED_CHECKS" -gt 0 ]; then
        green "✅ Branch protection active with $REQUIRED_CHECKS required checks"
        
        # Show required check names
        echo "   Required status checks:"
        echo "$PROTECTION_RULES" | jq -r '.required_status_checks.contexts[]?' | sed 's/^/     - /'
    else
        yellow "⚠️  No required status checks configured"
    fi
else
    yellow "⚠️  No branch protection rules found for main branch"
fi
echo ""

# Check 4: Open PRs with potential hanging issues
echo "$(blue "4. Scanning open PRs for automerge issues...")"
HANGING_PRS=0
            ...
            if COMMIT_STATUS=$(gh api "repos/$REPO_FULL/commits/$HEAD_SHA/status" 2>/dev/null); then
                ...
                if PR_CHECKS=$(gh pr view "$PR_NUMBER" --json statusCheckRollup 2>/dev/null); then
                    ...
                    if [ "$AUTO_MERGE_ENABLED" = "true" ] && [ "$OVERALL_STATE" = "pending" ] && [ "$COMPLETED_SUCCESSFUL_CHECKS" -gt 0 ]; then
                        red "🚨 POTENTIAL HANGING: PR #$PR_NUMBER"
                        ...
                        green "   ✅ PR #$PR_NUMBER: Automerge enabled, status: $OVERALL_STATE"
                    fi
                fi
            fi
...
echo "$(blue "5. Checking recent workflow health...")"
if RECENT_RUNS=$(gh run list --limit 5 --json status,conclusion,workflowName 2>/dev/null); then
    ...
    if [ "$FAILED_RUNS" -eq 0 ]; then
        green "✅ Recent workflow runs look healthy (no failures in last 5)"
    else
        yellow "⚠️  $FAILED_RUNS workflow failures in recent runs"
        echo "   Failed workflows might affect status check reporting"
    fi
else
    yellow "⚠️  Could not check recent workflow runs"
fi
echo ""

# Summary
echo "$(blue "📊 Health Check Summary")"
echo "======================="

if [ "$HANGING_PRS" -eq 0 ]; then
    green "✅ No automerge hanging issues detected"
    echo ""
    echo "$(blue "🛠️  Quick fixes for common issues:")"
    ...
    red "🚨 ALERT: $HANGING_PRS PR(s) potentially affected by automerge hanging"
    echo ""
    echo "$(blue "🚨 Immediate actions:")"
