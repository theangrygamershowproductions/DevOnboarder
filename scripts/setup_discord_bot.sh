...
if [[ ! -f ".env" ]]; then
    echo "FAILED .env file not found. Please run from the DevOnboarder root directory."
    exit 1
fi
...
echo "SEARCH Checking configuration..."
echo ""

# Verify required environment variables
if [[ -z "${DISCORD_CLIENT_ID:-}" ]]; then
    echo "FAILED DISCORD_CLIENT_ID not set in .env"
    echo "   Get this from: https://discord.com/developers/applications"
    exit 1
else
    echo "SUCCESS Discord Client ID: $DISCORD_CLIENT_ID"
fi

if [[ -z "${DISCORD_BOT_TOKEN:-}" ]]; then
    echo "FAILED DISCORD_BOT_TOKEN not set in .env"
    echo "   Get this from: https://discord.com/developers/applications"
    exit 1
else
    echo "SUCCESS Discord Bot Token: ${DISCORD_BOT_TOKEN:0:10}..."
fi
...
if [[ ! -d "bot" ]]; then
    echo "FAILED Bot directory not found"
    exit 1
fi

# Install bot dependencies if needed
echo "SYMBOL Checking bot dependencies..."
cd bot
...
echo ""
echo "CONFIG Available Commands:"
