...
validate_token_environment() {
    ...
    if [ -f "scripts/audit_token_usage.py" ]; then
        ...
        if [ -d ".venv" ]; then
            ...
            python scripts/audit_token_usage.py \
                ...
                echo "     Token audit encountered issues - check logs"
            }
        else
            echo "     Virtual environment not found - skipping Python token audit"
        fi
    fi
    ...
    if [ -n "${CI_ISSUE_AUTOMATION_TOKEN:-}" ]; then
        ...
        github_token_status="  GITHUB_TOKEN (policy violation - readonly only)"
    ...
    for token_check in "${specialized_tokens[@]}"; do
        ...
        if [ -n "${!token_name:-}" ]; then
            ...
            echo "     $description not configured ($token_name)"
        fi
    done
...
run_tests_with_artifacts() {
    ...
    if [ -f "scripts/audit_token_usage.py" ]; then
        ...
        echo "  Token audit script not available"
    fi
    ...
    if [ -d "bot" ]; then
        ...
        echo "  Bot directory not found, skipping bot tests"
    fi
...
- **Token Audit**: $([ "$token_audit_exit_code" -eq 0 ] && echo " COMPLIANT" || echo "  ISSUES DETECTED (exit code: $token_audit_exit_code)")
...
EOF
    ...
    if [ "$token_audit_exit_code" -ne 0 ]; then
        echo -e "${YELLOW}  Token governance issues detected${NC}"
