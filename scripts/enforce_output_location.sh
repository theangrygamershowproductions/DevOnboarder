...
check_root_pollution() {
    ...
    if find . -maxdepth 1 -name "node_modules" -type d 2>/dev/null | grep -v -E "^\\./bot/node_modules$|^\\./frontend/node_modules$|^\\./app/.*node_modules$" | grep -q .; then
        log_message "$RED" "❌ VIOLATION: Unexpected node_modules in root"
        find . -maxdepth 1 -name "node_modules" -type d | while read -r dir; do
            echo "   $dir → should be in bot/, frontend/, or app/ subdirectories only"
            violation_files+=("$dir")
        done
        violations=$((violations + 1))
    fi
