...
check_dry_run_config() {
    ...
    if [[ -f "$agent_file" ]]; then
        if grep -q "codex_dry_run: true" "$agent_file"; then
            ...
            echo "  Dry-run mode not explicitly set in agent configuration"
            echo "   Adding dry-run safety check..."
        fi

        if grep -q ".*dry-run mode" "$agent_file"; then
            echo " Dry-run warning notice present"
        else
            echo "  Dry-run warning notice not found"
        fi
    fi
}
...
validate_integration_readiness() {
    ...
    if [[ -f "bot/.env.dev" ]]; then
        ...
        echo "  Bot development environment not set up"
    fi
    ...
    if [[ -d ".codex" ]]; then
        ...
        echo "  Codex directory missing, creating structure..."
        mkdir -p .codex/agents .codex/state .codex/logs
    fi
...
run_integration_tests() {
    ...
    if [[ -f "bot/.env.dev" ]]; then
        ...
        echo "     Environment setup test had warnings (expected if first run)"
    fi
    ...
    if validate_integration_readiness; then
        ...
        echo "     Integration readiness test had warnings"
    fi
}
