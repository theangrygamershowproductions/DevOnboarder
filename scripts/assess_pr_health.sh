...
echo "HOSPITAL: PR Health Score: $HEALTH_PERCENTAGE%"
...
if [ "$HEALTH_PERCENTAGE" -ge 80 ]; then
    ...
    echo "    MEDIUM HEALTH: PR has significant value but needs attention"
    ...
    echo "    LOW HEALTH: Major issues present"
    echo "  SYNC: Recommendation: Fresh start with new PR might be more efficient"
...
if [ "$COMMIT_COUNT" -gt 10 ]; then
    echo "  GROW: HIGH COMMIT COUNT: Consider breaking into smaller PRs"
fi
