...
if ! bash scripts/analyze_service_dependencies.sh >/dev/null 2>&1; then
    echo " Warning: Service dependency analysis had issues, continuing..."
else
...
if ! python scripts/extract_service_interfaces.py >/dev/null 2>&1; then
    echo " Warning: Service interface extraction had issues, continuing..."
else
...
if ! bash scripts/catalog_shared_resources.sh >/dev/null 2>&1; then
    echo " Warning: Shared resources catalog had issues, continuing..."
else
...
if [ "$SERVICE_DIRS" -ge 4 ]; then
    ...
    echo "   MODERATE: Some service separation present"
...
if [ "$SHARED_DB_REFS" -lt 20 ]; then
    ...
    echo "   MEDIUM: Moderate database coupling"
    DB_COUPLING_SCORE=2
else
...
if [ -f "docs/service-api-contracts.md" ]; then
    ...
    if [ "$API_ENDPOINTS" -gt 20 ]; then
        ...
        echo "   DEVELOPING: Moderate API documentation"
...
if [ -f "bot/package.json" ] && grep -q "testTimeout" bot/package.json; then
    ...
    echo "   Jest timeout missing (potential CI issues)"
    TEST_CONFIG_SCORE=2
fi
...
if [ "$WORKFLOW_COUNT" -lt 10 ]; then
    ...
    echo "   MODERATE: Some CI/CD complexity"
...
if [ "$READINESS_PERCENTAGE" -ge 80 ]; then
    ...
    echo " RECOMMENDATION: PARTIALLY READY - ADDRESS KEY ISSUES"
    echo ""
    echo " Moderate readiness - address key issues before split"
    echo " Focus on improving lowest-scoring components"
    echo ""
