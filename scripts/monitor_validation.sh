...
echo "SYMBOL DevOnboarder CI Validation Monitor"
...
if [ -z "$LATEST_LOG" ]; then
    echo "FAILED No validation logs found in logs/"
    echo "IDEA Start validation first: bash scripts/validate_ci_locally.sh"
    exit 1
fi

echo "FILE Monitoring: $LATEST_LOG"
echo "SEARCH Use Ctrl+C to exit monitoring"
echo
echo "SYMBOL"

# Show current progress if validation is running
if pgrep -f "validate_ci_locally.sh" >/dev/null; then
    echo "GREEN Validation is currently RUNNING"
    echo "STATS Real-time progress:"
    echo
    tail -f "$LATEST_LOG"
else
    echo "RED Validation is NOT running"
    ...
    echo
    echo "IDEA TIP: Run 'bash scripts/validate_ci_locally.sh' to start new validation"
fi
